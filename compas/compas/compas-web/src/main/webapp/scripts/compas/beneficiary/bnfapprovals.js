 ...
 */
'use strict';
angular.module('app.bnfapprovals', []).controller("bnfapprovalsCtrl", ["$scope", "$filter","bnfapprovalSvc", "memberInquirySvc", "$bnfapprovalsValid", "$rootScope", "blockUI", "logger" ,"$location", function ($scope, $filter, bnfapprovalSvc,memberInquirySvc, $bnfapprovalsValid, $rootScope, blockUI, logger, $location) {
	var init;
	$scope.bioView = true;
	...
	$scope.loadMemberData = function () {
		...
		bnfapprovalSvc.GetRegMembers().success(function (response) {
	...
	$scope.loadVMemberData = function () {
		...
		bnfapprovalSvc.GetVerifiedMembers().success(function (response) {
	...
	$scope.verify=function(){
		...
		if($scope.selection.length > 0){
			...
			bnfapprovalSvc.UpdBnfStatus(bnf).success(function (response) {
				if (response.respCode == 200) {
					logger.logSuccess("Great! The Beneficiary Verified succesfully")
					$scope.memberInquiryEditMode = false;
					$scope.memberViewMode = true;
					$scope.selection=[];
					$scope.loadMemberData ();
	...
	bnfapprovalsSvc.GetVerifiedMembers = function ($scope) {
		return $http({
	...
	bnfapprovalsSvc.GetRegMembers = function ($scope) {
		return $http({
			url: '/compas/rest/member/gtRegMembers',
			method: 'GET',
			headers: { 'Content-Type': 'application/json' }
		});
	};
