...
angular.module('app.area', []).controller("areasCtrl", ["$scope", "$filter", "areaSvc","zoneSvc", "$areaValid", "$rootScope", "blockUI", "logger" , "$location", function ($scope, $filter, areaSvc,zoneSvc, $areaValid, $rootScope, blockUI, logger, $location) {
    ...
    $scope.loadZoneData = function () {
        ...
        zoneSvc.GetZones($rootScope.UsrRghts.linkId).success(function (response) {
    ...
    $scope.editArea = function (area) {
        if (!$filter('checkRightToEdit')($rootScope.UsrRghts.rightsHeaderList, "#" + $location.path())) {
            logger.log("Oh snap! You are not allowed to modify  Field Office.");
            return false;
        }
        $scope.header="Edit Field Office";
    ...
    $scope.addArea = function () {
        if (!$filter('checkRightToAdd')($rootScope.UsrRghts.rightsHeaderList, "#" + $location.path())) {
            logger.log("Oh snap! You are not allowed to create new Field Office.");
            return false;
        }
        $scope.header="Create Field Office";
        $scope.areaEditMode = true;
        $scope.areaId = 0;
        $scope.areaCode="";
        $scope.areaName = "";
    ...
    $scope.updArea = function () {
        ...
        if (!$areaValid($scope.areaCode)) {
            logger.logWarning("Opss! You may have skipped specifying the Field Office's Code. Please try again.")
            return false;
        }
        if (!$areaValid($scope.areaName)) {
            logger.logWarning("Opss! You may have skipped specifying the Field Office's Name. Please try again.")
            return false;
        }
        if ($scope.areaName.length > 50) {
            logger.logWarning("Opss!zone Name is reach to maximum length of 50 ")
            return false;
        }
        ...
        areaSvc.UpdArea(area).success(function (response) {
            if (response.respCode == 200) {
                logger.logSuccess("Great! The Field Office information was saved succesfully")
                $scope.areaId = 0;
                $scope.areaName = "";
                $scope.areaCode="";
                $scope.zoneSelect="";
