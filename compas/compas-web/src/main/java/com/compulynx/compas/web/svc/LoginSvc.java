...
//	}
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/safComLogin")
	public Response SafComDeviceLogin(SafComLogin user) {
		try {
			System.out.println("username####"+user.userName);
			SafComResponse loginUser = loginBal.SafComLogin(user.userName, user.password, user.deviceId, user.userType,user.licenseNumber);
			return Response.status(loginUser.respCode).entity(loginUser)
					.build();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/changePin")
	public Response ChangePin(SafComLogin login) {
		try {
			SafComResponse loginUser = loginBal.ChangePin(login);
			return Response.status(loginUser.respCode).entity(loginUser)
					.build();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/blockCard")
	public Response BlockCard(SafComLogin login) {
		try {
			SafComResponse loginUser = loginBal.BlockCard(login);
			return Response.status(loginUser.respCode).entity(loginUser)
					.build();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	@Consumes(MediaType.APPLICATION_JSON)
	@Path("/unBlockCard")
	public Response UnBlockCard(SafComLogin login) {
		try {
			SafComResponse loginUser = loginBal.UnblockCard(login);
			return Response.status(loginUser.respCode).entity(loginUser)
					.build();
		} catch (Exception ex) {
			ex.printStackTrace();
