 ...
 */
'use strict';
angular.module('app.pin', []).controller("pinCtrl", ["$scope", "$filter", "pinSvc","$pinValid", "$rootScope", "blockUI", "logger", "$location", function ($scope, $filter,pinSvc, $pinValid, $rootScope, blockUI, logger, $location) {
	
    var init;

	$scope.params = [];

    $scope.loadPinData = function () {
        $scope.pins = [], $scope.searchKeywords = "", $scope.filteredParam = [], $scope.row = "", $scope.deviceEditMode = false;
        paramSvc.GetPins().success(function (response) {
            return $scope.pins = response, $scope.searchKeywords = "", $scope.filteredParam = [], $scope.row = "", $scope.select = function (page) {
                var end, start;
