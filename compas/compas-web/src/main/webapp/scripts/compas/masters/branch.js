...
$scope.header="";
    ...
    $scope.editBranch = function (branch) {
    	if (!$filter('checkRightToEdit')($rootScope.UsrRghts.rightsHeaderList, "#" +
    			$location.path())) {
    		logger.log("Oh snap! You are not allowed to modify the Section.");
    		return false;
    	}
    	$scope.header="Edit Section";
    ...
    $scope.addBranch = function () {
    	...
    	if (!$filter('checkRightToAdd')($rootScope.UsrRghts.rightsHeaderList, "#" +
    			$location.path())) {
    		logger.log("Oh snap! You are not allowed to create Section.");
    		return false;
    	}
    	$scope.header="Create Section";
        $scope.branchEditMode = true;
    ...
    $scope.updBranch = function () {
        ...
        if (!$branchValid($scope.branchCode)) {
            logger.logWarning("Opss! You may have skipped specifying the Section's Code. Please try again.")
            return false;
        }
        if (!$branchValid($scope.branchName)) {
            logger.logWarning("Opss! You may have skipped specifying the Section's Name. Please try again.")
            return false;
        }
        ...
        branchSvc.UpdBranch(branch).success(function (response) {
            if (response.respCode == 200) {
                logger.logSuccess("Great! The Section information was saved succesfully")
